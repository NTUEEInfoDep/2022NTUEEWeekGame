...
export default class DinoGame extends GameRunner {
	...
	drawBackground() {
		...
		this.canvasCtx.drawImage(this.backgroundImage, 0, 0, this.width, this.height);
	}
	...
	drawClouds() {
		...
		if (this.frameCount % settings.cloudSpawnRate === 0) {
			...
			newCloud.y = randInteger(
				sprites.cloud.h + 10,
				this.state.dinoGroundOffset - sprites.cloud.h - sprites.dinoDuckLeftLeg.h
			);
	...
	drawBirds() {
		...
		if (this.frameCount % settings.birdSpawnRate === 0) {
			...
			if (randBoolean() && dino.powerUp !== "eater") {
				...
				newBird.y = dino.baseY - Bird.maxBirdHeight - 5 - sprites.dinoDuckLeftLeg.h / 2;
				birds.push(newBird);
			}
		}
	...
	drawFoods() {
		...
		if (dino.powerUp === "eater" && this.frameCount % settings.foodSpawnRate === 0) {
			...
			newFood.y = state.dino.baseY - newFood.height + this.foodHeightFunc(this.frameCount);
			foods.push(newFood);
		}
	...
	drawHighestScore() {
		...
		this.paintText("HI " + this.changeToString(this.highestScore), (25 * this.width) / 30, margin, {
			font: "PressStart2P",
			size: `${fontSize}px`,
			align: "right",
			baseline: "top",
